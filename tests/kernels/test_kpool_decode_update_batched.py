...
def _torch_reference(
    ...
    for b in range(B):
        for t in range(next_n):
            ...
            block = max(tail_slot_cpu[b][t], 0) // ring
...
def test_prefill_seed_honors_padded_tail_block_stride():
    """The tail shares the indexer's padded block stride in production."""
    kpool = 4
    num_blocks = 6
...
def test_prefill_seed_ignores_out_of_range_tail_block() -> None:
    """A stale/oversized physical tail id must not write outside the cache."""
    kpool = 4
    ring = 8
    num_blocks = 2
    block_elems = 2 * ring * HEAD_DIM
    sentinel = -321.0
    backing = torch.full(
        (4 * block_elems,), sentinel, dtype=torch.bfloat16, device="cuda"
    )
    tail = torch.as_strided(
        backing,
        size=(num_blocks, 2, ring, HEAD_DIM),
        stride=(block_elems, ring * HEAD_DIM, HEAD_DIM, 1),
    )
    key = torch.ones(1, HEAD_DIM, dtype=torch.bfloat16, device="cuda")
    score = torch.full_like(key, 2)
    out_of_range_block = 3
    tail_slot = torch.tensor(
        [out_of_range_block * ring + 1], dtype=torch.int32, device="cuda"
    )

    kpool_seed_tail_cache(tail, key, score, tail_slot, kpool, HEAD_DIM)
    torch.accelerator.synchronize()

    assert torch.all(backing == sentinel)
...
@pytest.mark.parametrize("pool_size", [4, 16])
@pytest.mark.skipif(current_platform.is_rocm(), reason="NVIDIA backport")
