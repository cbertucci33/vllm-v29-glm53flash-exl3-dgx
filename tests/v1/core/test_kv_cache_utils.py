...
def test_glm5_grouping_with_dflash_draft_layers():
    """A DFlash draft's plain full/sliding layers must ride the glm5 lane as
    their own groups with per-layer tensors, not trip page-size unification
    (the indexer page cannot be padded to a draft-influenced target page)."""
    model_config = ModelConfig(max_model_len=8192)
    vllm_config = VllmConfig(model_config=model_config)
    vllm_config.speculative_config = SimpleNamespace(
        method="dflash", parallel_drafting=False
    )
    ...
    kv_cache_spec["draft.layers.0.attn"] = draft_full
    kv_cache_spec["draft.layers.1.attn"] = draft_sw
    kv_cache_spec["draft.layers.2.attn"] = draft_sw
    ...
    assert sorted(len(g.layer_names) for g in draft_groups) == [1, 2]

    mla_page = kv_cache_spec["layers.3.attn"].page_size_bytes
    idx_page = kv_cache_spec["layers.3.indexer"].page_size_bytes
    draft_bytes = draft_full.page_size_bytes + 2 * draft_sw.page_size_bytes
    bytes_per_block = 11 * mla_page + 11 * idx_page + draft_bytes
    ...
    assert by_layer["draft.layers.0.attn"].size == bytes_per_block * 100
    assert by_layer["draft.layers.0.attn"].num_blocks == 100
    assert by_layer["draft.layers.1.attn"].layers == ["draft.layers.1.attn"]
    assert by_layer["draft.layers.1.attn"].size == bytes_per_block * 100
    assert by_layer["draft.layers.1.attn"].num_blocks == 100

    # Without a dflash speculative config the extra layers must not be
    # silently adopted by the glm5 lane.
...
def test_glm5_dflash_ring_is_a_fixed_allocation(monkeypatch):
    model_config = ModelConfig(max_model_len=8192)
    vllm_config = VllmConfig(model_config=model_config)
    vllm_config.speculative_config = SimpleNamespace(
        method="dflash", parallel_drafting=False
    ...
    for index in range(5):
        kv_cache_spec[f"draft.layers.{index}.attn"] = draft_sw

    groups = kv_cache_utils.get_kv_cache_groups(vllm_config, kv_cache_spec)
    layout = kv_cache_utils._glm5_next_tensor_layout(groups)
    assert layout is not None
    draft_groups = layout[-1]
    assert draft_groups[0].is_eagle_group
    assert not draft_groups[0].enable_kv_transfer
    assert not draft_groups[0].kv_cache_spec.prefix_cacheable
    assert draft_groups[0].kv_cache_spec.private_ring

    ring_size = (
        draft_sw.max_admission_blocks_per_request(
            max_in_flight_tokens=vllm_config.max_in_flight_tokens,
            max_model_len=8192,
        )
        + 1
    )
    ring_pages = 1 + vllm_config.scheduler_config.max_num_seqs * ring_size
    assert kv_cache_utils._dflash_ring_pages(vllm_config, draft_groups) == ring_pages
    assert kv_cache_utils.dflash_ring_layer_names(vllm_config, groups) == {
        f"draft.layers.{index}.attn" for index in range(5)
    }

    # A DFlash spec by itself is not enough to make a fixed ring. The allocator
    # only rings draft groups inside the recognized GLM hybrid layout, so the
    # pool copy/zero exclusion helper must make the same decision.
    non_glm_groups = [KVCacheGroupSpec(["other.draft.attn"], draft_sw)]
    assert kv_cache_utils._glm5_next_tensor_layout(non_glm_groups) is None
    assert kv_cache_utils._dflash_ring_pages(vllm_config, non_glm_groups) is None
    assert not kv_cache_utils.dflash_ring_layer_names(vllm_config, non_glm_groups)

    mla_page = kv_cache_spec["layers.3.attn"].page_size_bytes
    idx_page = kv_cache_spec["layers.3.indexer"].page_size_bytes
    bytes_per_block = 11 * mla_page + 11 * idx_page
    assert kv_cache_utils._pool_bytes_per_block(groups, vllm_config) == bytes_per_block

    fixed = ring_pages * 5 * draft_sw.page_size_bytes
    config = kv_cache_utils.get_kv_cache_config_from_groups(
        vllm_config, groups, fixed + bytes_per_block * 100 + 1
    )
    assert config.num_blocks == 100
    shared_blocks_per_request = sum(
        (group.kv_cache_spec.max_memory_usage_bytes(vllm_config)
         + group.kv_cache_spec.page_size_bytes - 1)
    ...
    assert get_max_concurrency_for_kv_cache_config(
        vllm_config, config
    ) == 100 / shared_blocks_per_request
    by_layer = _tensor_by_layer(config)
    for index in range(5):
        tensor = by_layer[f"draft.layers.{index}.attn"]
        assert tensor.layer_stride == ring_pages * draft_sw.page_size_bytes
        assert tensor.offset + tensor.layer_stride <= tensor.size

    monkeypatch.setenv("VLLM_DFLASH_KV_RING", "0")
    groups = kv_cache_utils.get_kv_cache_groups(vllm_config, kv_cache_spec)
    layout = kv_cache_utils._glm5_next_tensor_layout(groups)
    assert layout is not None
    draft_groups = layout[-1]
    assert not draft_groups[0].kv_cache_spec.private_ring
    assert kv_cache_utils._dflash_ring_pages(vllm_config, draft_groups) is None
    config = kv_cache_utils.get_kv_cache_config_from_groups(
        vllm_config, groups, fixed + bytes_per_block * 100 + 1
    )
    tensor = _tensor_by_layer(config)["draft.layers.0.attn"]
    assert tensor.layer_stride == config.num_blocks * draft_sw.page_size_bytes


_GLM5_PORT_LAYOUT_SKIP = pytest.mark.skip(
    reason=(
        "ported with glm5_next from newer-main; asserts the upstream "
        "KVCacheTensor layout. This fork uses its shared_by-alias dialect."
    )
)
