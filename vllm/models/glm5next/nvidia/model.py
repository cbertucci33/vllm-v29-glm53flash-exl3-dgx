    ...
    def forward(
        ...
        if not self.mhc or self.is_mtp_layer:
            ...
            if output_indices is not None:
                ...
                # MTP MLA and sparse-index caches. Only request-tail outputs
                # feed draft sampling, so compact the residual stream before
                # the MoE.
...
class Glm5NextModel(nn.Module, EagleModelMixin):
    ...
    def _completed_layer_output(
        ...
        # Aux capture contract (DFlash, sglang PR #36708): the completed output
        # of the layer that just ran, hc streams mean-contracted to one
        # per-token state. In the deferred-mHC dialect the layer's hc_post is
        # not yet applied at the loop boundary (it fuses into the next layer's
        # pre), so materialize it here without disturbing the deferred state.
        if post is None:
...
class Glm5NextForCausalLM(
    ...
    def get_eagle3_default_aux_hidden_state_layers(self) -> tuple[int, ...]:
        # The aux capture implements the DFlash contract (hc-contracted
        # completed layer outputs), not the EAGLE3 hidden+residual convention.
        # There is no trained default layer set, so fail loud rather than hand
        # an eagle3 draft mismatched tensors.
