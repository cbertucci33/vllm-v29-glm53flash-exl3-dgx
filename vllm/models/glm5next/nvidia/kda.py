    ...
    def __init__(
        ...
        if self.kda_prefill_backend == "flashkda":
            ...
            workspace_size = torch.ops._flashkda_C.get_workspace_size(
                max_tokens, num_heads, max_sequences
            )
            self._flashkda_buffer_specs = (
                (
    ...
    def _forward(
        ...
        if attn_metadata_narrowed.num_prefills > 0:
            ...
            if self.kda_prefill_backend == "flashkda":
                ...
                # q_ns is still flattened as [tokens, heads * head_dim] here;
                # FlashKDA requires out to match the rearranged query shape
                # [1, tokens, heads, head_dim].
                flashkda_out = workspace_out[:, : q_ns.shape[0]]
                assert flashkda_out.shape == (
                    1,
                    q_ns.shape[0],
                    self.local_num_heads,
                    self.head_dim,
                )
                # FlashKDA fuses the bounded gate and the beta sigmoid
                # in-kernel: pass raw bf16 beta and raw g1, unlike the chunk
                # path below which wants the pre-sigmoided fp32 beta.
                (
                ...
                (
                    ...
                    # kernels don't sigmoid); beta_ns is raw bf16 from
                    # forward.
