...
class FlashInferMLASparseTRTLLMBackend(_FlashInferMLASparseBackendBase):
    ...
    def supports_combination(
        ...
        if kv_cache_dtype == "fp8_ds_mla":
            return (
                "FLASHINFER_MLA_SPARSE SM100 does not support fp8_ds_mla "
                "kv-cache dtype"
            )

        # FlashInfer MLA sparse SM100 kernel requires qk_nope_head_dim in [128, 192].
...
class FlashInferMLASparseSM120Backend(_FlashInferMLASparseBackendBase):
    ...
    def supports_combination(
        ...
        if not use_sparse:
            return "FLASHINFER_MLA_SPARSE_SM120 requires sparse MLA"
        if head_size != 512:
            return (
                "FLASHINFER_MLA_SPARSE_SM120 requires GLM53_NOPE "
                f"head_size=512; got {head_size}"
            )
        if kv_cache_dtype != "fp8_ds_mla":
            return (
                "FLASHINFER_MLA_SPARSE_SM120 requires the packed fp8_ds_mla "
                f"KV cache layout; got {kv_cache_dtype!r}"
            )
        vllm_config = get_current_vllm_config()
        if vllm_config.model_config is None:
            return "FLASHINFER_MLA_SPARSE_SM120 requires a model configuration"

        hf_text_config = vllm_config.model_config.hf_text_config
        model_type = getattr(hf_text_config, "model_type", None)
        if model_type != "glm5_next_text":
            return (
                "FLASHINFER_MLA_SPARSE_SM120 currently supports only "
                f"GLM-5 Next text models; got model_type={model_type!r}"
            )

        required_geometry = {
            "index_topk": 2048,
            # This is the model's pre-absorption Q dimension.  The query
            # passed to FlashInfer is widened to kv_lora_rank=512 below.
            "qk_nope_head_dim": 256,
            "qk_rope_head_dim": 0,
            "kv_lora_rank": 512,
        }
        for field, required in required_geometry.items():
            actual = getattr(hf_text_config, field, None)
            if actual != required:
                return (
                    "FLASHINFER_MLA_SPARSE_SM120 requires GLM53_NOPE "
                    f"{field}={required}; got {actual!r}"
                )
        return None
